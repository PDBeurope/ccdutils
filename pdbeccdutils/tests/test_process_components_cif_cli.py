...
from pdbeccdutils.tests.tst_utilities import cif_filename, test_cut_down_components_cif
...
class TestCutDownComponentsCif:
    ...
    def test_correct_atom_naming_in_files(pipeline_wd, id_, name, alt_name):
        ...
        """
        alts = [
            os.path.join(pipeline_wd, id_[0], id_, "{}_ideal_alt.pdb".format(id_)),
    ...
    def test_cml_files(pipeline_wd, chem_comp_id):
        ...
        """
        path = os.path.join(
            pipeline_wd, chem_comp_id[0], chem_comp_id, "{}.cml".format(chem_comp_id)
        )
